    ...
    def exec(self, access_token, subscription_ids):
        ...
        with progress.bar.Bar(progress_text, max = len(subscriptions)) as bar:
            for subscription in subscriptions:
                loadbalancers = utils.get_all_resources_of_type_within_subscription(self._access_token, subscription, self._resource_type)
                api_versions = utils.get_api_version_for_resource_type(self._access_token, subscription, self._resource_type)

                for loadbalancer in loadbalancers:
                    spinner.next()
                    loadbalancer_content = utils.get_resource_content_using_multiple_api_versions(self._access_token, loadbalancer, api_versions, spinner)
