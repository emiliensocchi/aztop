    ...
    def exec(self, access_token, subscription_ids):
        ...
        with progress.bar.Bar(progress_text, max = len(subscriptions)) as bar:
            for subscription in subscriptions:
                resources_xyz = utils.get_all_resources_of_type_within_subscription(self._access_token, subscription, self._resource_type)
                ...
                for resource_xyz in resources_xyz:
                    spinner.next()
                    resource_xyz_content = utils.get_resource_content_using_multiple_api_versions(self._access_token, resource_xyz, api_versions, spinner)
                    ...
                    resource_xyz_network_exposure = utils.get_resource_network_exposure(self._access_token, subscription, resource_xyz_properties, spinner)
